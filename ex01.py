...
if __name__ == "__main__":
    ...
    with open(os.path.join(os.getcwd(), "files", "nomes.txt"), "r", encoding="utf-8") as arquivo:
        ...
        # Utilizarems list-comprehension para formatar o conteúdo da lista
        # Removemos o caractere especial '\n' das strings
        conteudo = [item.replace("\n", "") for item in conteudo]

        # Convertendo todas as letras da palavra para minúsculas
        conteudo = [item.lower() for item in conteudo]


        # Convertendo as strings da lista para o formato snake case
        conteudo = [item.replace(" ", "_") for item in conteudo]


        # Criando um dicionário que irá armazenar as informações
        repeticao_nomes = {}
        ...
        # Criar a estrutura final que mostrará os resultados
        lista_final = []
        ...
        lista_final = sorted(lista_final, key=lambda x: x.get("vezes"), reverse=True)
